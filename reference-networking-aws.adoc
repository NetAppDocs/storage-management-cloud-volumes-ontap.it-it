...
Durante la distribuzione di Cloud Volumes ONTAP, il campo proxy del mediatore AWS accetta un indirizzo server. Se si inserisce un fully qualified domain name (FQDN) invece di un indirizzo IP, l'FQDN non verrà risolto a meno che la AWS VPC non sia configurata con un DNS server personalizzato. Questo perché il DNS fornito di default da AWS risolve solo i record DNS pubblici e gli host interni AWS e non può risolvere FQDN esterni come i proxy server dei clienti. Se questo scenario si applica a te, hai due opzioni:

* Aggiungi l'indirizzo IP del proxy server direttamente nel campo proxy del mediatore invece del fully qualified domain.
* Configura un set di opzioni DHCP nella tua VPC che includa gli indirizzi dei tuoi server DNS personalizzati. Ciò consente ad AWS di risolvere il fully qualified domain quando lo inserisci nel campo proxy server durante la distribuzione. Consulta la  https://docs.aws.amazon.com/vpc/latest/userguide/DHCPOptionSet.html["Documentazione AWS"^] per ulteriori dettagli.
...
TIP: Cerchi informazioni sull'agente Console? https://docs.netapp.com/us-en/console-setup-admin/reference-ports-aws.html["Visualizza le regole del gruppo di sicurezza per l'agente della console"^]
...
* https://docs.netapp.com/us-en/console-setup-admin/concept-install-options-aws.html["Visualizza i requisiti di rete per l'agente della console"^]
