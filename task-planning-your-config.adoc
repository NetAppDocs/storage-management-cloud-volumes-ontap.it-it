...
NOTE: Il tiering dei dati su Amazon Simple Storage Service (Amazon S3) non è supportato se il tuo sistema Cloud Volumes ONTAP si trova in una AWS Local Zone, perché l'accesso ai bucket Amazon S3 al di fuori della Local Zone comporta una latenza maggiore e influisce sulle attività di Cloud Volumes ONTAP.
...
link:reference-configs-aws.html#aws["Visualizza i dischi predefiniti per i dati di sistema Cloud Volumes ONTAP in AWS"].
