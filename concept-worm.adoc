...
La funzionalità WORM è disponibile senza costi aggiuntivi per gli abbonamenti al marketplace e per le licenze Bring-Your-Own License (BYOL). Contatta il tuo NetApp sales representative per aggiungere WORM alla tua licenza attuale.
...
È possibile utilizzare un'applicazione per eseguire il commit dei file in WORM tramite NFS o CIFS, oppure utilizzare la ONTAP CLI per eseguire automaticamente il commit dei file in WORM. È inoltre possibile utilizzare un file WORM appendibile per conservare i dati scritti in modo incrementale, come le informazioni di log. Per informazioni, consultare  https://docs.netapp.com/us-en/ontap/snaplock/commit-files-worm-state-manual-task.html#commit-files-to-worm-manually["Esegui il commit dei file in WORM utilizzando ONTAP SnapLock"^].

Dopo aver attivato lo storage WORM su un sistema Cloud Volumes ONTAP, devi usare la ONTAP CLI per tutta la gestione dello storage WORM. Per istruzioni, consulta la link:https://docs.netapp.com/us-en/ontap/snaplock/index.html["Documentazione ONTAP su SnapLock"^].
...
* link:concept-licensing-charging.html["Licenze e addebiti in Cloud Volumes ONTAP"]
