...
+
.Per le implementazioni in modalità privata, assicurati di soddisfare questi prerequisiti aggiuntivi:
[%collapsible]
====
...
** Se esegui lo strumento in modalità privata, assicurati che, insieme alle altre API, tu abbia abilitato l'API Cloud Build. https://docs.netapp.com/us-en/console-setup-admin/task-install-agent-google-gcloud.html#step-5-enable-google-cloud-apis["Abilita le API di Google Cloud"^].
** Assicurati di aver completato le configurazioni di rete e creato il pool di worker per le distribuzioni in modalità privata. Fai riferimento a link:reference-networking-gcp.html#infrastructure-manager-configuration-for-private-mode-deployments["Configurazione di Infrastructure Manager per le implementazioni in modalità privata"].
...
| cloudbuild.googleapis.com | 443 | TCP | USCITA | Operazioni di build richieste solo per la modalità privata 
...
| pypi.org | 443 | TCP | USCITA | Indice dei pacchetti Python 


| files.pythonhosted.org | 443 | TCP | USCITA | Download dei pacchetti Python 


| checkpoint-api.hashicorp.com | 443 | TCP | USCITA | Verifica della versione di Terraform 


| download.docker.com | 443 | TCP | USCITA | repository APT Docker 


| security.ubuntu.com | 80/443 | TCP | USCITA | Aggiornamenti di sicurezza Ubuntu 


| *.gce.archive.ubuntu.com | 80 | TCP | USCITA | Mirror dei pacchetti Ubuntu 
|===
...
----
+
.Includi autorizzazioni aggiuntive per le implementazioni in modalità privata
[%collapsible]
====
...
. Scarica lo strumento di conversione da  https://mysupport.netapp.com/site/tools/tool-eula/gcp-cvo-dm-to-im-convert-tool/download["Sito di supporto NetApp"^] utilizzando le tue credenziali NSS: `wget <download link from NetApp Support site>`
. Estrai il file TAR scaricato: `unzip <downloaded file name>`
...
. Imposta Python: `sudo update-alternatives --install /usr/bin/python3 python3 /usr/bin/python3.11 2`
. Installa pip3 per installare il pacchetto: `python3.11 -m ensurepip --upgrade`
. Installa Terraform:
+
[source, cli]
----
...
----
. Installa Google Cloud SDK (gcloud)
+
[source, cli]
----
...
----
+
.Esegui lo strumento nelle distribuzioni in modalità privata
[%collapsible]
====
Specifica il parametro  `--worker-pool` per eseguire lo strumento nelle distribuzioni in modalità privata. Per la configurazione del pool di worker, consulta link:reference-networking-gcp.html#infrastructure-manager-configuration-for-private-mode-deployments["Configurazione di Infrastructure Manager per le implementazioni in modalità privata"].
...
.Dopo aver finito
Lo strumento visualizza un elenco di tutti i sistemi Cloud Volumes ONTAP e i dettagli delle SVM. Al termine dell'esecuzione, puoi vedere lo stato di tutti i sistemi convertiti. Ogni sistema convertito appare nella Google Console sotto Infrastructure Manager in un formato _<system-name-imdeploy>_, indicando che la Console ora utilizza le API di Infrastructure Manager per gestire quel sistema Cloud Volumes ONTAP.


CAUTION: Dopo la conversione, non eliminare l'oggetto di distribuzione per Deployment Manager nella Google Cloud Console. Questo oggetto di distribuzione contiene informazioni che potrebbero essere necessarie per eseguire il rollback del sistema convertito.
