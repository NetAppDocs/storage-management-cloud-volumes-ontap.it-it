...
Per informazioni sulla configurazione dei server proxy, consulta  https://docs.netapp.com/us-en/console-setup-admin/task-configuring-proxy.html["Configurare l'agente della console per utilizzare un server proxy"^].
...
La Console crea gruppi di sicurezza di Azure che includono le regole inbound e outbound per consentire il corretto funzionamento di Cloud Volumes ONTAP  https://docs.netapp.com/us-en/console-setup-admin/reference-ports-azure.html["Visualizza le regole del gruppo di sicurezza per l'agente della console"^].
...
Non ti consigliamo di modificare i gruppi di sicurezza predefiniti o di usare gruppi di sicurezza personalizzati. Tuttavia, se proprio devi farlo, tieni presente che il processo di distribuzione richiede che il sistema Cloud Volumes ONTAP abbia pieno accesso alla propria sottorete. Dopo il completamento della distribuzione, se decidi di modificare il gruppo di sicurezza di rete, assicurati di mantenere aperte le porte del cluster e le porte di rete HA, perché la rete HA si basa su una comunicazione illimitata tra i due nodi. Inoltre, gli indirizzi IP dei nodi vengono definiti automaticamente durante la distribuzione. Questo garantisce una comunicazione senza interruzioni all'interno del cluster Cloud Volumes ONTAP (comunicazione any-to-any tra i nodi).
...
* https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-azure.html["Visualizza i requisiti di rete per l'agente della console"^]
