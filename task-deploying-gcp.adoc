...
[[licensing]]
* Un agente NetApp Console attivo e funzionante.
+
** Dovresti avere un  https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-google.html["Agente console associato al tuo sistema"^].
** https://docs.netapp.com/us-en/console-setup-admin/concept-connectors.html["Dovresti essere pronto a lasciare l'agente della console sempre in esecuzione"^].
...
. Se ti viene richiesto, https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-google.html["creare un agente Console"^].
...
| Nome dell'account di servizio | Se vuoi usare link:concept-data-tiering.html["livelli di dati"] o  https://docs.netapp.com/us-en/data-services-backup-recovery/concept-backup-to-cloud.html["NetApp Backup and Recovery"^] con Cloud Volumes ONTAP, devi abilitare *Service Account* e selezionare un account di servizio che abbia il ruolo predefinito Storage Admin. link:task-creating-gcp-service-account.html["Scopri come creare un account di servizio"^]. 
...
Se non vedi altri progetti nell'elenco a discesa, significa che non hai ancora associato l'account di servizio ad altri progetti. Vai alla Google Cloud Console, apri il servizio IAM e seleziona il progetto. Aggiungi l'account di servizio con il ruolo che usi per la Console a quel progetto. Dovrai ripetere questo passaggio per ogni progetto.


NOTE: Questo è l'account di servizio che hai configurato per la Console, link:https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-google.html["come descritto in questa pagina"^].

Fare clic su *Aggiungi abbonamento* per associare le credenziali selezionate a un abbonamento.

Per creare un sistema Cloud Volumes ONTAP pay-as-you-go, devi selezionare un progetto Google Cloud associato a un abbonamento a Cloud Volumes ONTAP dal marketplace di Google Cloud. Consulta  https://docs.netapp.com/us-en/console-setup-admin/task-adding-gcp-accounts.html["Associazione di un abbonamento al marketplace alle credenziali di Google Cloud"^].
...
. *Posizione e connettività*: seleziona la regione e la zona di Google Cloud per il tuo sistema, scegli una policy firewall e conferma la connettività di rete allo storage di Google Cloud per il data tiering.
+
Nella tabella seguente vengono descritti i campi per i quali potrebbe essere necessaria una guida:
...
. *Pacchetti preconfigurati*: seleziona uno dei pacchetti per distribuire rapidamente un sistema Cloud Volumes ONTAP oppure fai clic su *Create my own configuration*. I pacchetti preconfigurati variano in base alla versione di Cloud Volumes ONTAP selezionata. Ad esempio, per Cloud Volumes ONTAP 9.18.1 e versioni successive, la Console mostra pacchetti con VM C3, inclusi dischi Hyperdisk Balanced. Puoi modificare le configurazioni, come i parametri IOPS e throughput, in base alle esigenze del tuo carico di lavoro.
+
...
+
Per assistenza nella scelta del tipo e delle dimensioni del disco, consulta link:task-planning-your-config-gcp.html#size-your-system-in-google-cloud["Dimensiona il tuo sistema in Google Cloud"^].

. *Cache flash, velocità di scrittura e WORM*:
+
.. Abilita *Flash Cache* o scegli *Normale* o *Alta* velocità di scrittura, se necessario.
+
Scopri di più su link:reference-configs-gcp.html#flash-cache-support["Cache flash"] e link:concept-write-speed.html["velocità di scrittura"].

+

NOTE: L'opzione di velocità di scrittura *Alta* offre un'elevata velocità di scrittura e un'unità di trasmissione massima (MTU) più elevata, pari a 8.896 byte. Inoltre, l'MTU più elevata di 8.896 richiede la selezione di VPC-1, VPC-2 e VPC-3 per la distribuzione. Per ulteriori informazioni su VPC-1, VPC-2 e VPC-3, consulta  https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/reference-networking-gcp.html#requirements-for-the-console-agent["Regole per VPC-1, VPC-2 e VPC-3"^].
...
** Per assistenza con l'aggiunta di un account Google Cloud, consulta  https://docs.netapp.com/us-en/console-setup-admin/task-adding-gcp-accounts.html["Configurazione e aggiunta di account Google Cloud per la suddivisione in livelli dei dati con 9.6"^].
...
** Se disabiliti il data tiering, puoi abilitarlo sugli aggregati successivi, ma dovrai disattivare il sistema e aggiungere un account di servizio dal Google Cloud Console.
+
...
| Server NTP | Seleziona *Usa dominio Active Directory* per configurare un server NTP utilizzando il DNS di Active Directory. Se devi configurare un server NTP utilizzando un indirizzo diverso, allora dovresti usare l'API. Per informazioni, consulta la  https://docs.netapp.com/us-en/console-automation/index.html["Documentazione sull'automazione NetApp Console"^] per i dettagli. Nota che puoi configurare un server NTP solo quando crei un server CIFS. Non è configurabile dopo che hai creato il server CIFS. 
|===
...
CAUTION: Al termine del processo di distribuzione, non modificare le configurazioni di Cloud Volumes ONTAP generate dal sistema nel portale Google Cloud, come i tag di sistema e le etichette impostate nelle risorse Google Cloud. Qualsiasi modifica apportata a queste configurazioni potrebbe causare comportamenti imprevisti o perdita di dati.
...
. *Dettagli del sistema*: Seleziona un progetto, specifica un nome per il cluster, seleziona facoltativamente un Service Account, aggiungi facoltativamente delle etichette e infine specifica le credenziali.
+
Nella tabella seguente vengono descritti i campi per i quali potrebbe essere necessaria una guida:
...
| Progetto Google Cloud e abbonamento Marketplace  a| 
Seleziona il progetto Google e l'abbonamento che vuoi usare per questo sistema. Questo è l'account di servizio che hai configurato per la Console. Per informazioni, consulta la link:https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-google.html["Documenti di configurazione della console"^]. La Console usa il progetto per creare il sistema Cloud Volumes ONTAP e l'abbonamento per determinare fatturazione e utilizzo. Se non vedi altri progetti nell'elenco a discesa, significa che non hai ancora associato l'account di servizio ad altri progetti. Vai alla Google Cloud Console, apri il servizio IAM e seleziona il progetto. Aggiungi l'account di servizio con il ruolo che usi per la Console a quel progetto. Dovrai ripetere questo passaggio per ogni progetto.

Fare clic su *Aggiungi abbonamento* per associare le credenziali selezionate a un abbonamento.

Per creare un sistema Cloud Volumes ONTAP pay-as-you-go, devi selezionare un progetto Google Cloud associato a un abbonamento a Cloud Volumes ONTAP dal Google Cloud Marketplace. Consulta il documento di configurazione della Console: https://docs.netapp.com/us-en/console-setup-admin/task-adding-gcp-accounts.html["Associazione di un abbonamento al marketplace alle credenziali di Google Cloud"^].
...
| Nome dell'account di servizio | Se hai intenzione di utilizzare i servizi link:concept-data-tiering.html["NetApp Cloud Tiering"] o  https://docs.netapp.com/us-en/data-services-backup-recovery/concept-backup-to-cloud.html["Backup e ripristino"^], devi abilitare l'interruttore *Account di servizio* e poi selezionare l'Account di servizio che ha il ruolo predefinito Storage Admin. 


| Etichette | Le etichette sono metadati per le tue risorse Google Cloud.  La console aggiunge le etichette al sistema Cloud Volumes ONTAP e alle risorse Google Cloud associate al sistema.  È possibile aggiungere fino a quattro etichette dall'interfaccia utente durante la creazione di un sistema, per poi aggiungerne altre dopo la creazione.  Tieni presente che l'API non ti limita a quattro etichette quando crei un sistema.  Per informazioni sulle etichette, fare riferimento a https://cloud.google.com/compute/docs/labeling-resources["Documentazione di Google Cloud: Etichettatura delle risorse"^] . 


| Nome utente e password ONTAP | Queste sono le credenziali per l'account amministratore del cluster Cloud Volumes ONTAP .  È possibile utilizzare queste credenziali per connettersi a Cloud Volumes ONTAP tramite ONTAP System Manager o ONTAP CLI.  Mantenere il nome utente predefinito _admin_ oppure modificarlo con un nome utente personalizzato. 
|===
. *Servizi*: seleziona i servizi che desideri utilizzare su questo sistema.  Per selezionare Backup e ripristino o per utilizzare NetApp Cloud Tiering, è necessario aver specificato l'account di servizio nel passaggio 3.
+
...
. *Topologia di distribuzione*: Scegli più zone di disponibilità (AZ) (consigliato) o una singola AZ per la configurazione HA.
+
link:concept-ha-google-cloud.html["Scopri di più sui modelli di distribuzione HA"^] .

. *Configurazione di rete e connettività*: Seleziona le regioni e le zone pertinenti. Seleziona quattro VPC diverse per la configurazione HA, una subnet in ciascuna VPC e poi scegli una policy firewall.
+

NOTE: Cloud Volumes ONTAP 9.19.1 e versioni successive supportano link:concept-ha-google-cloud.html["dischi condivisi per configurazioni HA a zona di disponibilità singola"], rendendo l'istanza del mediatore opzionale. In questa configurazione, non devi inserire una zona mediatrice, la VPC corrispondente e la subnet per la replica dei dati.
...
+
Se scegli uno dei pacchetti, devi solo specificare un volume e poi rivedere e approvare la configurazione. I pacchetti preconfigurati variano in base alla versione di Cloud Volumes ONTAP selezionata e ai tipi di configurazione. Ad esempio, se selezioni una distribuzione a singola AZ per Cloud Volumes ONTAP 9.19.1 o versioni successive, puoi vedere pacchetti preconfigurati con macchine della serie C3 e dischi Hyperdisk Balanced. Per le versioni precedenti e per più AZ, puoi comunque selezionare macchine n2-standard e Persistent Disks. Puoi modificare le configurazioni, come i parametri IOPS e throughput, in base alle esigenze del tuo carico di lavoro.
...
NOTE: L'opzione *Alta* velocità di scrittura e un'unità di trasmissione massima (MTU) più elevata, pari a 8.896 byte, sono disponibili con i tipi di istanza n2-standard-16, n2-standard-32, n2-standard-48 e n2-standard-64. Inoltre, l'MTU più elevata di 8.896 richiede la selezione di VPC-1, VPC-2 e VPC-3 per la distribuzione. L'alta velocità di scrittura e un MTU di 8.896 dipendono dalle funzionalità e non possono essere disabilitati singolarmente all'interno di un'istanza configurata. Per ulteriori informazioni su VPC-1, VPC-2 e VPC-3, consulta  https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/reference-networking-gcp.html#requirements-for-the-connector["Regole per VPC-1, VPC-2 e VPC-3"^].
...
| Server NTP | Seleziona *Usa dominio Active Directory* per configurare un server NTP utilizzando il DNS di Active Directory. Se devi configurare un server NTP utilizzando un indirizzo diverso, allora dovresti usare l'API. Consulta il  https://docs.netapp.com/us-en/console-automation/index.html["Documentazione sull'automazione NetApp Console"^] per i dettagli. Tieni presente che puoi configurare un server NTP solo durante la creazione di un server CIFS. Non è configurabile dopo che hai creato il server CIFS. 
|===
. *Profilo di utilizzo, tipo di disco e criterio di suddivisione in livelli*: scegli se abilitare le funzionalità di efficienza dell'archiviazione e modificare il criterio di suddivisione in livelli del volume, se necessario.
+
Per maggiori informazioni, fare riferimento alink:task-planning-your-config-gcp.html#choose-a-volume-usage-profile["Scegli un profilo di utilizzo del volume"^] ,link:concept-data-tiering.html["Panoramica della suddivisione in livelli dei dati"^] , E https://kb.netapp.com/Cloud/Cloud_Volumes_ONTAP/What_Inline_Storage_Efficiency_features_are_supported_with_CVO#["KB: Quali funzionalità di Inline Storage Efficiency sono supportate da CVO?"^]
