...
A partire dalla versione 9.12.1 ONTAP , NVMe-TCP è supportato da tutti i provider cloud.  Cloud Volumes ONTAP supporta NVMe-TCP come protocollo a blocchi per le VM di archiviazione (SVM) durante la distribuzione e installa automaticamente le licenze NVMe richieste.

NetApp Console non fornisce alcuna funzionalità di gestione per NVMe-TCP.
