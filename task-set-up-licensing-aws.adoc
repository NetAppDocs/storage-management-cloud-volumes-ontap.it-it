...
== Impostazione della fatturazione preferita per l'inserzione AWS autonoma

Se disponi di un abbonamento AWS autonomo collegato al tuo account AWS, puoi pagare Cloud Volumes ONTAP tramite tale abbonamento. Se disponi anche di abbonamenti NetApp Intelligent Services (NIS), puoi pagare i servizi dati tramite il pacchetto associato all'abbonamento NIS. Per gli abbonamenti esistenti, NetApp consiglia di passare all'approccio con abbonamento autonomo per una migliore gestione dei costi di utilizzo di Cloud Volumes ONTAP.

.Informazioni su questo compito
Se sei già un cliente con abbonamenti NIS associati all'account AWS, NetApp consiglia di passare all'approccio di abbonamento autonomo per una migliore gestione dei costi relativi all'utilizzo di Cloud Volumes ONTAP. Questa è la procedura completa:
...
. (Se applicabile) Converti il tuo pacchetto Professional in un pacchetto Essentials e imposta la preferenza di fatturazione. Consulta  https://docs.netapp.com/us-en/console-licenses-subscriptions/task-manage-capacity-licenses.html#change-charging-methods["Modificare le modalità di addebito per le licenze di capacità"^] per informazioni sulla conversione del tuo pacchetto Professional in un pacchetto Essentials.
. Continua a fatturare i servizi dati tramite il tuo abbonamento NIS.


.Passi
. Nel pannello di sinistra della NetApp Console, selezionare *Amministrazione > Licenses and subscriptions > Preferenze di fatturazione > Cloud Volumes ONTAP*.
. Nella scheda AWS, seleziona l'abbonamento autonomo e fai clic sull'icona delle opzioni image:screenshot_gallery_options.gif["icona del kebab"].
. Nella finestra Modifica configurazione, seleziona il numero di abbonamento dell'inserzione autonoma nel campo *Credenziali dell'account AWS*.
. Fai clic su *Configura*. Puoi vedere l'abbonamento come opzione preferita per la fatturazione di Cloud Volumes ONTAP in AWS.
. Quando si implementa un nuovo sistema Cloud Volumes ONTAP, selezionare il pacchetto Essentials per la fatturazione tramite l'abbonamento autonomo. Per informazioni, consulta  https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-quick-deploy-aws.html["Implementa Cloud Volumes ONTAP con Quick create in AWS"^].


Ripeti gli stessi passaggi per l'abbonamento NIS nella scheda *Servizi dati* per utilizzare l'abbonamento NIS per i tuoi servizi dati. Quando implementi un nuovo sistema Cloud Volumes ONTAP, seleziona il pacchetto Professional per la fatturazione tramite l'abbonamento NIS.
...
* Una licenza acquistata da NetApp (Bring-Your-Own License o BYOL)
...
. https://docs.netapp.com/us-en/console-setup-admin/task-adding-nss-accounts.html#add-an-nss-account["Aggiungi il tuo account del sito di supporto NetApp alla console"^]
+
...
+
La licenza deve essere disponibile dalla Console prima di poterla utilizzare con Cloud Volumes ONTAP. Se necessario, puoi link:https://docs.netapp.com/us-en/console-licenses-subscriptions/task-manage-data-services-licenses.html#add-a-license["aggiungere manualmente la licenza alla Console"^].
...
TIP: Puoi gestire gli abbonamenti ad AWS Marketplace associati ai tuoi account AWS dalla pagina Impostazioni > Credenziali. https://docs.netapp.com/us-en/console-setup-admin/task-adding-aws-accounts.html["Scopri come gestire i tuoi account e abbonamenti AWS"^]
...
Seleziona l'offerta Freemium per utilizzare Cloud Volumes ONTAP gratuitamente con una capacità di provisioning fino a 500 GiB. link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/concept-licensing.html#free-trials["Scopri di più sull'offerta Freemium"^].
...
.. Nella pagina *Dettagli e credenziali*, fai clic su *Modifica credenziali > Aggiungi abbonamento* e segui le istruzioni per abbonarti all'offerta con pagamento in base al consumo in AWS Marketplace.
+
Non ti verrà addebitato alcun costo tramite l'abbonamento al marketplace a meno che tu non superi 500 GiB di capacità allocata, momento in cui il sistema viene automaticamente convertito a link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/concept-licensing.html#packages["Pacchetto essenziale"^].
