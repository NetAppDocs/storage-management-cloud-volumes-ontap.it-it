...
---
= Distribuisci Cloud Volumes ONTAP utilizzando le opzioni avanzate in Azure
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
+
** Dovresti avere un  https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-azure.html["Agente console associato al tuo sistema"^].
...
. Se ti viene richiesto https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-azure.html["creare un agente Console"^].
...
| Tag del gruppo di risorse | I tag sono metadati per le risorse di Azure. Quando si immettono tag in questo campo, la Console li aggiunge al gruppo di risorse associato al sistema Cloud Volumes ONTAP . Quando si crea un sistema, è possibile aggiungere fino a quattro tag dall'interfaccia utente, per poi aggiungerne altri dopo la creazione. Tieni presente che l'API non ti limita a quattro tag quando crei un sistema. Per informazioni sui tag, fare riferimento a https://learn.microsoft.com/en-us/azure/azure-resource-manager/management/tag-resources["Documentazione di Microsoft Azure: utilizzo dei tag per organizzare le risorse di Azure"^] . 
...
| Modifica credenziali | Puoi scegliere credenziali Azure e una sottoscrizione Azure diverse da usare con questo sistema Cloud Volumes ONTAP. Devi associare una sottoscrizione Azure Marketplace alla sottoscrizione Azure selezionata per poter distribuire un sistema Cloud Volumes ONTAP pay-as-you-go. https://docs.netapp.com/us-en/console-setup-admin/task-adding-azure-accounts.html["Scopri come aggiungere le credenziali"^]. 
|===
. *Servizi*: abilita o disabilita i singoli servizi che desideri o non desideri utilizzare con Cloud Volumes ONTAP.
+
** https://docs.netapp.com/us-en/data-services-data-classification/concept-cloud-compliance.html["Scopri di più sulla NetApp Data Classification"^]
...
TIP: Se l'account Azure che stai utilizzando ha la  https://docs.netapp.com/us-en/console-setup-admin/reference-permissions-azure.html["permessi richiesti"^], la Console rimuove le risorse Cloud Volumes ONTAP da un gruppo di risorse, in caso di errore di distribuzione o eliminazione.
...
| Utilizzare esistente | Se scegli un gruppo di sicurezza esistente, deve soddisfare i requisiti di Cloud Volumes ONTAP. link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/reference-networking-azure.html#security-group-rules["Visualizza il gruppo di sicurezza predefinito"^]. 
|===
...
NOTE: Se per la versione selezionata è disponibile una Release Candidate, una General Availability o una patch più recente, Console aggiorna il sistema a tale versione durante la creazione dell'ambiente di lavoro. Ad esempio, l'aggiornamento avviene se si seleziona Cloud Volumes ONTAP 9.16.1 P3 e la versione 9.16.1 P4 è disponibile. L'aggiornamento non avviene da una release all'altra, ad esempio dalla 9.15 alla 9.16.
...
+
Per assistenza nella scelta del tipo e delle dimensioni del disco, consulta link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-planning-your-config-azure.html#size-your-system-in-azure["Dimensionamento del sistema in Azure"^].
...
+
Questa opzione è disponibile solo per alcuni tipi di VM. Per scoprire quali tipi di VM sono supportati, consulta link:reference-configs-azure.html#ha-pairs["Configurazioni supportate per licenza per coppie HA"].
...
** La dimensione del disco si riferisce a tutti i dischi nell'aggregato iniziale e a tutti gli aggregati aggiuntivi creati dalla Console quando si utilizza l'opzione di provisioning semplice.  È possibile creare aggregati che utilizzano dimensioni del disco diverse utilizzando l'opzione di allocazione avanzata.
+
Per assistenza nella scelta della dimensione del disco, fai riferimento a link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-planning-your-config-azure.html#size-your-system-in-azure["Dimensiona il tuo sistema in Azure"^].
...
+
link:reference-configs-azure.html#ha-pairs["Scopri di più sulle configurazioni supportate in Azure"^].
...
.Link correlati
* link:task-planning-your-config-azure.html["Pianificazione della configurazione Cloud Volumes ONTAP in Azure"^]
* link:task-deploy-cvo-azure-mktplc.html["Distribuisci Cloud Volumes ONTAP in Azure da Azure Marketplace"^]
