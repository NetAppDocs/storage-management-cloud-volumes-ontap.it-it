...
* Per AWS, avrai bisogno di un endpoint VPC per Amazon Simple Storage Service (Amazon S3). <<Requirements to tier cold data to AWS S3,Saperne di più>>.
...
=== Requisiti per il tiering dei dati cold su Amazon S3

Assicurarsi che Cloud Volumes ONTAP abbia una connessione ad Amazon S3. Il modo migliore per fornire tale connessione è creare un endpoint VPC per il servizio S3. Per istruzioni, fare riferimento a https://docs.aws.amazon.com/AmazonVPC/latest/UserGuide/vpce-gateway.html#create-gateway-endpoint["Documentazione AWS: creazione di un endpoint gateway"^].
...
----
Il ruolo personalizzato include le autorizzazioni per impostazione predefinita. https://docs.netapp.com/us-en/console-setup-admin/reference-permissions-azure.html["Visualizza l'autorizzazione di Azure per l'agente della console"^]
...
+
Se non selezioni un account di servizio durante la distribuzione, dovrai arrestare Cloud Volumes ONTAP, accedere a Google Cloud Console e quindi collegare l'account di servizio alle istanze di Cloud Volumes ONTAP. Puoi quindi abilitare il tiering dei dati come descritto nella sezione successiva.
...
+
Per assistenza sulla replica dei dati, fai riferimento a  https://docs.netapp.com/us-en/data-services-replication/task-replicating-data.html["Replica dei dati da e verso il cloud"^].
...
. Fai clic sull'icona image:icon-action.png["Un'icona che mostra tre puntini orizzontali per azioni aggiuntive"] dell'agente Console che gestisce il tuo sistema Cloud Volumes ONTAP.
