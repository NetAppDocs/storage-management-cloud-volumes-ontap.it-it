...
Dischi:: I dati dei clienti risiedono nei blob di pagina di Premium Storage. Ogni nodo ha accesso allo storage dell'altro nodo. È inoltre necessario ulteriore storage per link:reference-configs-azure.html#azure-ha-pair["dati di avvio, root e core"].
...
NOTE: A partire da Cloud Volumes ONTAP 9.15.0P1, i blob di pagine di Azure non sono più supportati per le nuove distribuzioni di coppie HA. Se attualmente usi i blob di pagine di Azure in distribuzioni di coppie HA esistenti, puoi migrare a tipi di istanze di macchine virtuali più recenti nelle VM della serie Edsv4 e nelle VM della serie Edsv5. link:reference-configs-azure.html#ha-pairs["Scopri di più sulle configurazioni supportate in Azure"].
...
Dischi:: I dati dei clienti risiedono su dischi gestiti con ridondanza locale (LRS). Ogni nodo ha accesso allo storage dell'altro nodo. È inoltre necessario ulteriore storage per link:reference-configs-azure.html#azure-ha-pair["dati di avvio, root, root partner, core e NVRAM"].
...
Dischi:: I dati dei clienti risiedono su dischi gestiti con ridondanza di zona (ZRS). Ogni nodo ha accesso allo storage dell'altro nodo. È inoltre necessario ulteriore storage per link:reference-configs-azure.html#azure-ha-pair["dati di avvio, root, root partner e core"].
Account di archiviazione:: Gli account di archiviazione vengono utilizzati per le distribuzioni basate su dischi gestiti per gestire i log diagnostici e la suddivisione in livelli nell'archiviazione BLOB.
...
NOTE: Cloud Volumes ONTAP utilizza bilanciatori di carico in modalità attivo-passivo per le sonde. In Azure, solo il nodo attivo risponde a una sonda. Il nodo passivo potrebbe apparire inattivo con una X. Questo è il comportamento previsto e non deve destare preoccupazione.

.Link correlato
link:task-azure-high-availability-mode.html["Abilita la modalità ad alta disponibilità per Cloud Volumes ONTAP in Azure"^]
