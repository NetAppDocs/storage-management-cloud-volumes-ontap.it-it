...
Si noti che un LIF di gestione opzionale per una VM di storage richiede un indirizzo IP privato su un sistema a nodo singolo e su una coppia HA in una singola AZ. Questo LIF di gestione fornisce una connessione a strumenti di gestione come SnapCenter.
...
+
** sistema a nodo singolo: è necessario un IP privato secondario non utilizzato.
+
...
== Crea una VM di archiviazione su un sistema a nodo singolo

Questi passaggi creano una nuova VM di storage su un sistema a nodo singolo. È necessario un indirizzo IP privato per creare un LIF NAS e un altro indirizzo IP privato facoltativo se si desidera creare un LIF di gestione.
...
** `-service-policy default-data-iscsi`indica che un indirizzo IP non migra tra i nodi.
