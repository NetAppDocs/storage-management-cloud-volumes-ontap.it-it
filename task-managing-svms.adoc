...
[role="lead"]
Una storage VM (SVM) è una macchina virtuale in esecuzione all'interno di ONTAP che fornisce servizi di storage e dati ai tuoi client. Potresti conoscerla come _SVM_ o _vserver_. Cloud Volumes ONTAP è configurato di default con una storage VM, ma alcune configurazioni supportano storage VM aggiuntive.
