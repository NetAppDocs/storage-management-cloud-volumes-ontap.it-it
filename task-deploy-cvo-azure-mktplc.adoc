...
summary:  
---
= Distribuisci Cloud Volumes ONTAP da Azure Marketplace
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Puoi usare la distribuzione diretta di Azure Marketplace per distribuire rapidamente e facilmente Cloud Volumes ONTAP. Da Azure Marketplace, puoi distribuire rapidamente Cloud Volumes ONTAP in pochi clic ed esplorare le sue funzionalità principali e capacità nel tuo ambiente.
...
Il sistema Cloud Volumes ONTAP distribuito tramite distribuzione diretta da Azure Marketplace presenta queste caratteristiche. Nota che le funzionalità di un'istanza autonoma distribuita tramite Azure Marketplace cambiano quando viene rilevata nella NetApp Console.

* L'ultima versione Cloud Volumes ONTAP (9.16.1 o successiva).
* Per Cloud Volumes ONTAP è disponibile una licenza gratuita fino a 500 GiB di capacità allocata, senza supporto NetApp e senza data di scadenza. Per capacità superiori a 500 GiB è necessaria una licenza valida e potrebbero essere applicati dei costi. Per accedere all’esperienza completa di Cloud Volumes ONTAP e alle funzionalità di gestione avanzate, collega il tuo sistema Cloud Volumes ONTAP alla Console. Il rilevamento e la gestione tramite la Console richiedono un abbonamento valido ad Azure Marketplace associato al tuo account Console.
...
.Prima di iniziare
* Se vuoi individuare e gestire il sistema distribuito nella Console, assicurati che l'account della Console sia associato a un abbonamento valido di Azure Marketplace. L'individuazione del sistema nella Console richiede un abbonamento valido.
...
I sistemi Cloud Volumes ONTAP distribuiti direttamente tramite Azure Marketplace non includono il supporto di NetApp. Se si verificano problemi durante la distribuzione, puoi risolverli e affrontarli in autonomia.

.Passi
. Sul sito di Azure Marketplace, vai a *Diagnostica di avvio > Registro seriale*.
...
Puoi individuare i sistemi Cloud Volumes ONTAP che hai distribuito utilizzando la distribuzione diretta di Azure Marketplace e gestirli nella pagina *Systems* della Console. L'agente della Console scopre i sistemi, li aggiunge, applica le licenze necessarie e sblocca tutte le funzionalità della Console per questi sistemi. La configurazione HA originale in una singola AZ con PSSD v2 Managed Disks viene mantenuta e il sistema viene registrato alla stessa Azure subscription e resource group della distribuzione originale.

.Informazioni su questo compito
Quando individui i sistemi Cloud Volumes ONTAP distribuiti tramite la distribuzione diretta di Azure Marketplace, l'agente della Console esegue queste attività:
...
.Prima di iniziare
Assicurati che la distribuzione sia completa su Azure Marketplace. L'agente della Console può individuare i sistemi solo quando la distribuzione è completa e sono disponibili per l'individuazione.
...
CAUTION: Durante la fase di discovery, potresti vedere messaggi di errore, ma puoi ignorarli fino al completamento del processo di discovery. Non modificare le configurazioni di Cloud Volumes ONTAP generate dal sistema nel portale Azure Marketplace durante la discovery, soprattutto i tag di sistema. Qualsiasi modifica apportata a queste configurazioni può causare comportamenti imprevisti del sistema.
...
* https://docs.netapp.com/us-en/console-licenses-subscriptions/task-manage-subscriptions.html["Aggiungi un nuovo abbonamento al marketplace"^]
