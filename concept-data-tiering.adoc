...
Quando abiliti il tiering dei dati in Google Cloud, Cloud Volumes ONTAP utilizza dischi persistenti come tier di performance per i dati attivi e un bucket di Google Cloud Storage come tier di capacità per i dati inattivi. Puoi selezionare un tier di storage quando crei un sistema e cambiarlo in qualsiasi momento successivo. Per informazioni sulla modifica della classe di storage, consulta link:task-tiering.html["Suddividi i dati inattivi in un archivio di oggetti a basso costo"].
...
--
Classi di archiviazione:: Google Cloud utilizza  https://cloud.google.com/storage/docs/storage-classes["classi di archiviazione"^] per gestire i costi e la disponibilità dei dati. Ti consigliamo di selezionare una classe di archiviazione in base alla frequenza prevista di accesso ai dati, perché accedere ai dati comporta costi più elevati. La classe di archiviazione predefinita per i dati a livelli è _Archiviazione Standard_. A meno che non venga modificata manualmente, tutti i dati inattivi mantengono la classe di archiviazione predefinita. Per modificare la classe di archiviazione dei dati inattivi, consulta  https://docs.cloud.google.com/storage/docs/managing-lifecycles["Documentazione Google Cloud: Gestione del ciclo di vita degli oggetti"^].
+
--
La classe di archiviazione per la suddivisione in livelli dei dati è a livello di sistema e non per volume.
