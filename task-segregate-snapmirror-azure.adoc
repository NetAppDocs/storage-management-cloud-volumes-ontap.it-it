...
---
= Segrega il traffico SnapMirror in Cloud Volumes ONTAP in Azure
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
----
.. Salva l'indirizzo IP privato. Questo indirizzo IP, <new_added_nic_primary_addr>, viene utilizzato per creare un LIF intercluster in <<step-second,dominio broadcast e LIF intercluster per la nuova NIC>>.
...
== [[step-second]]Passaggio 2: Creare un nuovo IPspace, dominio broadcast e LIF intercluster per la nuova NIC
...
----
. Per i sistemi a nodo singolo, la porta appena aggiunta è _e0b_. Per le distribuzioni a coppia HA con dischi gestiti, la porta appena aggiunta è _e0d_. Per le distribuzioni a coppia HA con page blob, la porta appena aggiunta è _e0e_. Usa il nome del nodo, non il nome della VM. Trova il nome del nodo eseguendo  `node show`.
+
[source, cli]
----
