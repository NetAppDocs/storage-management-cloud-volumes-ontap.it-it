...
* Gestione dello storage a oggetti ONTAP S3: le funzionalità di gestione dello storage ONTAP S3 in Cloud Volumes ONTAP sono disponibili solo in System Manager e non nella Console.
...
. Fai clic sull'icona image:icon-action.png["Un'icona che mostra tre puntini orizzontali per azioni aggiuntive"] per l'agente della console richiesto e seleziona *Modifica agente della console*.
...
* https://docs.netapp.com/us-en/ontap/concept_administration_overview.html["Scopri l'amministrazione del cluster con ONTAP System Manager"^]
